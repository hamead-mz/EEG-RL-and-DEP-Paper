...
def generate_figure(data, config):
    ...
    for Pi in range(3):
        ...
        for tii, time_inv in enumerate(config['graph_args']['times']):
            ...
            BrainNetwork.DrawStaticGraph(0.05, window = time_inv, ax = ax, show_labels = True, xlims = [-1.5, 1.5], ylims = [-1, 1.1], DirectionBias = 0, Colors = config['graph_args']['color_events'][Pi], FigConstants = config['graph_args']['n_fig_consts'])
...
def build_figure():
    ...
    handle_figure(fig, fig_name = 'supp-04')
